	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
    	...
    	if (cmd.getName().equalsIgnoreCase("simplecommandblocker")) {
    		if (args.length < 1) {
    			...
    			if (sender.hasPermission("simplecommandblocker.admin") || sender.hasPermission("scb.admin")) {
    				if (args.length >= 2) {
    					...
    					if (scm.getCommand(args[1]).equals(null)) {
    						Utilities.sendSender(sender, ChatColor.RED + plugin.getMessages().get("not_a_command").toString().replaceAll("<cmd>", args[1].toString()));
    					} else {
    						Boolean tryAdd = Utilities.configListAdd("allowed_commands", args[1].toString());
    						if (tryAdd) {
    							Utilities.sendSender(sender, ChatColor.GREEN + plugin.getMessages().get("cmd_allowed").toString().replaceAll("<cmd>", args[1].toString()));
    							...
    							Utilities.sendSender(sender, ChatColor.GREEN + plugin.getMessages().get("already_allowed").toString().replaceAll("<cmd>", args[1].toString()));
    						}
    					}
    				} else {
    					help(sender);
    			...
    			if (sender.hasPermission("simplecommandblocker.admin") || sender.hasPermission("scb.admin")) {
    				if (args.length >= 2) {
						Boolean tryRem = Utilities.configListRemove("allowed_commands", args[1].toString());
						if (tryRem) {
							Utilities.sendSender(sender, ChatColor.GREEN + plugin.getMessages().get("cmd_blocked").toString().replaceAll("<cmd>", args[1].toString()));
							if (plugin.getMinecraftVersion().contains("MC: 1.13")) {
								Bukkit.getServer().getOnlinePlayers().forEach(uPlayer -> {
									uPlayer.updateCommands();
								});
							...
							Utilities.sendSender(sender, ChatColor.GREEN + plugin.getMessages().get("already_blocked").toString().replaceAll("<cmd>", args[1].toString()));
						}
    				} else {
    					help(sender);
    				}
				...
				if (sender.hasPermission("simplecommandblocker.admin") || sender.hasPermission("scb.admin")) {
    				...
    				if (!trustList.isEmpty()) {
    					for (String t_uuid : trustList) {
    						Utilities.sendSender(sender, Bukkit.getOfflinePlayer(UUID.fromString(t_uuid)).getName().toString());
    					}
    				} else {
